    ...
    public partial class frmChiTietKiemTraHuHong : Form
    {
        ...
        private void LoadDanhSachThietBi()
        {
            ...
            if (dt != null)
            {
                ...
                dataGridView1.Columns["GhiChu"].DefaultCellStyle.WrapMode = DataGridViewTriState.True;
                dataGridView1.Columns["GhiChu"].DefaultCellStyle.Alignment = DataGridViewContentAlignment.MiddleLeft;
                dataGridView1.Columns["GhiChu"].DefaultCellStyle.Padding = new Padding(10, 8, 10, 8); // Lề cho dễ đọc
                dataGridView1.Columns["GhiChu"].Width = 200;
        ...
        private void frmChiTietKiemTraHuHong_Load(object sender, EventArgs e)
        {
            ...
            DataRow phieu = PhieuKiemTraHuHongDAL.Instance.GetPhieuByMa(_maPhieu); 
            if (phieu != null)
            {
        ...
        private void dataGridView1_DataBindingComplete(object sender, DataGridViewBindingCompleteEventArgs e)
        {
            ...
            foreach (DataGridViewRow row in dataGridView1.Rows)
            {
                if (!row.IsNewRow) 
                {
                    // Tính chiều cao tối ưu dựa trên nội dung tất cả các cell (ưu tiên cột GhiChu vì đã wrap)
                    int preferredHeight = row.GetPreferredHeight(row.Index,
                        DataGridViewAutoSizeRowMode.AllCells, true);

                    // Đặt chiều cao, tối thiểu 60px để đẹp, +10px padding
                    row.Height = Math.Max(preferredHeight + 10, 60);
                }
            }
        }
    }
